		...
		if mi.Notif != "" {
			...
			if mi.Method == "session/prompt" {
				...
				caseBody = append(
					...
					Id("entry").Op(":=").Op("&").Id("sessionPromptCancel").Values(Dict{Id("cancel"): Id("cancel")}),
					Id("a").Dot("mu").Dot("Lock").Call(),
					If(List(Id("prev"), Id("ok")).Op(":=").Id("a").Dot("sessionCancels").Index(Id("string").Call(Id("p").Dot("SessionId"))), Id("ok")).Block(Id("prev").Dot("cancel").Call()),
					Id("a").Dot("sessionCancels").Index(Id("string").Call(Id("p").Dot("SessionId"))).Op("=").Id("entry"),
					Id("a").Dot("mu").Dot("Unlock").Call(),
				)
				...
				caseBody = append(
					...
					Id("a").Dot("mu").Dot("Lock").Call(),
					If(Id("a").Dot("sessionCancels").Index(Id("string").Call(Id("p").Dot("SessionId"))).Op("==").Id("entry")).Block(
						Id("delete").Call(Id("a").Dot("sessionCancels"), Id("string").Call(Id("p").Dot("SessionId"))),
					),
					Id("a").Dot("mu").Dot("Unlock").Call(),
